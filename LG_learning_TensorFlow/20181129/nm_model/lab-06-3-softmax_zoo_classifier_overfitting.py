...
with tf.Session() as sess:
    ...
    for train_idx, val_idx in kf.split(x_data, y_data) :
        train_xdata = x_data[train_idx]
        train_ydata = y_data[train_idx]
        test_xdata = x_data[val_idx]
        test_ydata = y_data[val_idx]

        for step in range(2000):
            sess.run(optimizer, feed_dict={X: train_xdata, Y: train_ydata})
            if step % 100 == 0:
                loss, acc = sess.run([cost, accuracy], feed_dict={X: train_xdata, Y: train_ydata})
                print("Step: {:5}\tLoss: {:.3f}\tAcc: {:.2%}".format(step, loss, acc))

        # Let's see if we can predict
        pred = sess.run(prediction, feed_dict={X: test_xdata})
        # y_data: (N,1) = flatten => (N, ) matches pred.shape
        for p, y in zip(pred, test_ydata):
